...
class DroneNode(Node):
    ...
    def on_teammate_state(self, msg: String):
        ...
        try:
            ...
            self.get_logger().debug(f'Updated state for drone {drone_id}')
    ...
    def _detect_drones(self):
        """Detect other drones"""
        all_positions = self.coordinator.get_all_friendly_positions()
    
        # Only keep drones that are NOT teammates (i.e., potential enemies)
        self.detected_drones = {
            drone_id: pos for drone_id, pos in all_positions.items()
        ...
        for drone_id, position in self.detected_drones.items():
            ...
            if classification == "ENEMY":
                self.get_logger().warn(
                    f'[Drone {self.drone_id}] ⚠️  THREAT DETECTED: Enemy {drone_id} at {distance:.2f}m, threat_level={threat_level}')
